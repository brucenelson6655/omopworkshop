...
-- MAGIC %python
-- MAGIC ehr_dfs = {}
-- MAGIC for path,name in [(f.path,f.name) for f in dbutils.fs.ls(ehr_path) if f.name !='README.txt']:
-- MAGIC   df_name = name.replace('.csv','')
-- MAGIC   ehr_dfs[df_name] = spark.read.csv(path, header=True, inferSchema=True)
-- MAGIC   ehr_dfs[df_name].createOrReplaceTempView(df_name)
-- MAGIC 
-- MAGIC # Display number of records in each table
-- MAGIC out_str="<h2>There are {} tables in this collection with:</h2><br>".format(len(ehr_dfs))
-- MAGIC for k in ehr_dfs:
-- MAGIC   out_str+='{}: <i style="color:Tomato;">{}</i> records <br>'.format(k.upper(),ehr_dfs[k].count())
-- MAGIC 
-- MAGIC displayHTML(out_str)

-- COMMAND ----------

CREATE
OR REPLACE TEMPORARY VIEW IP_VISITS AS WITH CTE_END_DATES AS (
  SELECT
    patient,
    encounterclass,
...
CREATE
OR REPLACE TEMPORARY VIEW ER_VISITS AS
SELECT
  T2.encounter_id,
  T2.patient,
  T2.encounterclass,
...
INSERT
  OVERWRITE condition_occurrence
select
  row_number() over(
    order by
      p.person_id
...
from
  conditions c
  inner join source_to_standard_vocab_map srctostdvm on srctostdvm.source_code = c.code
  ...
  left join source_to_source_vocab_map srctosrcvm on srctosrcvm.source_code = c.code
  and srctosrcvm.source_vocabulary_id = 'SNOMED'
  left join final_visit_ids fv on fv.encounter_id = c.encounter
  inner join person p on c.patient = p.person_source_value;
